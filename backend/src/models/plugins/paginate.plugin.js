...
const paginate = (schema) => {
  ...
  schema.statics.paginate = async function (filter, options) {
    ...
    for (const value in filter) {
      if (!mongoose.isValidObjectId(filter[value]) && (!['level', '_id'].includes(value)) && value !== "$or") {
        if (value === "queryRange") {
          findFilter["$and"] = filter[value]["start"] && filter[value]["end"] ? [{$expr: {$gte: [{$convert: {input: "$" + filter[value]["field"], to: "decimal"}}, filter[value]["start"]]}}, {$expr: {$lte: [{$convert: {input: "$" + filter[value]["field"] , to: "decimal"}}, filter[value]["end"]]}}] :
            filter[value]["start"] ? [{$gte: [{$convert: {input: "$" + filter[value]["field"], to: "decimal"}}, filter[value]["start"]] }] : [{$lte: [{$convert: {input: "$" + filter[value]["field"], to: "decimal"}}, filter[value]["end"]]}];
